...
async function main() {
    ...
    const appartements = sqlite.prepare("SELECT * FROM Appartement").all();
    ...
    const paiements = sqlite.prepare("SELECT * FROM Paiement").all();
    ...
    const annonces = sqlite.prepare("SELECT * FROM Annonce").all();
    ...
    const interventions = sqlite.prepare("SELECT * FROM DemandeIntervention").all();
    ...
    for (const row of documents) {
        await pg.document.create({
            data: {
                id: row.id,
                nom: row.nom,
                type: ["BAIL", "AVIS", "REGLEMENT", "FACTURE", "AUTRE"].includes(row.type)
                    ? row.type
                    : "AUTRE",
                description: row.description,
                url: row.url,
