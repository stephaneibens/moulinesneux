...
export async function GET() {
  ...
  const items = prisma.demandeIntervention.findMany({ include: { locataire: { select: { nom: true, prenom: true, appartement: { numero: true } } } } })
...
export async function POST(request: Request) {
  ...
  const val = prisma.demandeIntervention.create({
  ...
  if (val.locataireId) {
    const loc = prisma.user.findUnique({ id: val.locataireId }, { include: { appartement: true } })
