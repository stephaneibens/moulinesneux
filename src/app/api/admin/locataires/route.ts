...
export async function GET() {
  ...
  const locataires = prisma.user.findMany({
...
export async function POST(request: Request) {
  ...
  const newUser = prisma.user.create({ email, nom, prenom, telephone, appartementId: appartementId || null, passwordHash, role: 'LOCATAIRE', dateEcheanceBail: dateEcheanceBail || null, garantieLocative: garantieLocative ? parseFloat(garantieLocative) : null, actif: actif !== undefined ? actif : true })
